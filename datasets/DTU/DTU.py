...
class DTU(Dataset):
    def __init__(self, config, mode):
        ...
        self.robust_train = config['data']['robust_train'] if is_train and 'robust_train' in config['data'] else False
        self.lightings=range(7)
        self.mode = mode
        self.nsrcs = config['model']['params']['nsrc']
        self.nscale = config['model']['params']['nscale'] 
    ...
    def parse_cameras(self, path):
        ...
        if self.mode in ('train', 'val'): # note the loaded camera model is for 1/4 original resolution
            intr_mat[:2] *= 4.
    ...
    def __getitem__(self, idx):
        ...
        sample['depth_values'] = np.array([depth_min, depth_max], np.float32)

        # proj inv at 1/8 resolution 
        level = 3
        ref_in_l3 = torch.Tensor(np.array(ref_intrinsics)).clone() # 3x3
        ref_in_l3[:2, :3] /= (2**level)
        src_ins_l3 = torch.Tensor(np.array(src_intrinsics)).clone() # Nx3x3
        src_ins_l3[:, :2, :3] /= (2**level)
        ref_proj = torch.matmul(ref_in_l3, torch.Tensor(np.array(ref_extrinsics))[:3, :3])
        src_projs = torch.matmul(src_ins_l3, torch.Tensor(np.array(src_extrinsics))[:, :3, :3])
        ref_proj_inv_l3 = ref_proj.inverse().cpu().numpy()
        src_projs_inv_l3 = src_projs.inverse().cpu().numpy()
        sample['ref_proj_inv_l3'] = ref_proj_inv_l3     # 3X3
        sample['src_projs_inv_l3'] = src_projs_inv_l3   # 3X3
        sample['proj_invs_l3'] = torch.cat([ref_proj.inverse().unsqueeze(0), src_projs.inverse()], dim=0).cpu().numpy()
        
        sample['imgs'] = np.concatenate([np.expand_dims(sample["ref_img"], 0), np.moveaxis(np.array(src_imgs),3,1)], axis=0)

        # cam to cam proj at 1/8 res
        ref_proj_l3 = torch.Tensor(np.array(ref_extrinsics)).clone() # 4x4
        ref_proj_l3[:3, :4] = torch.matmul(ref_in_l3, torch.Tensor(np.array(ref_extrinsics))[:3, :4])
        ref_proj_inv_l3 = ref_proj_l3.inverse().numpy()
        src_proj_l3 = torch.Tensor(np.array(src_extrinsics)).clone()
        src_proj_l3[:, :3, :4] = torch.matmul(src_ins_l3, torch.Tensor(np.array(src_extrinsics)[:, :3, :4]))
        sample['proj_l3'] = torch.matmul(src_proj_l3, torch.Tensor(ref_proj_inv_l3).view(1,4,4))

        # cam to cam proj at 1/4 resolution
        level = 2
        ref_in_l = torch.Tensor(np.array(ref_intrinsics)).clone() # 3x3
        ref_in_l[:2, :3] /= (2**level)
        src_in_l = torch.Tensor(np.array(src_intrinsics)).clone() # 3x3
        src_in_l[:, :2, :3] /= (2**level)
        ref_proj_l2 = torch.Tensor(np.array(ref_extrinsics)).clone() # 4x4
        ref_proj_l2[:3, :4] = torch.matmul(ref_in_l, torch.Tensor(np.array(ref_extrinsics))[:3, :4]) # 3X4
        ref_proj_inv_l2 = ref_proj_l2.inverse().numpy()
        src_proj_l2 = torch.Tensor(np.array(src_extrinsics)).clone()
        src_proj_l2[:, :3, :4] = torch.matmul(src_in_l, torch.Tensor(np.array(src_extrinsics)[:, :3, :4]))
        sample['proj_l2'] = torch.matmul(src_proj_l2, torch.Tensor(ref_proj_inv_l2).view(1,4,4))

        # depth_samples
        h, w = ref_img.shape[:2]
        height4, width4 = h//4, w//4
        num_depth = self.ndepths
        inverse_depth_base = 1 / depth_max    # Bx1
        inverse_depth_multiplier = 1 / depth_min - 1 / depth_max  # Bx1
        inverse_samps = inverse_depth_base + inverse_depth_multiplier * (num_depth - torch.arange(0, num_depth) - 1) / num_depth
        depth_samps_flat = 1 / inverse_samps.view(-1,1,1)   # Nx1x1
        sample['depth_samps_flat'] = depth_samps_flat

        if self.mode == 'test':
            sample["fn"] = scene_name + '/{:0>8}'.format(ref_view)
            sample['ref_img_n'] = torch.from_numpy(np.ascontiguousarray(ref_img)) * 255
            sample['src_imgs_n'] = [torch.from_numpy(np.ascontiguousarray(img)) * 255 for img in src_imgs] # TODO 只为了 vis
