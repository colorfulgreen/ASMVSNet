...
from models.module import FeatureNet, ContextNet
from models.layers.deform_layers import DeformConv2dAdp, DeformConv2d
from models.layers.FMT import EncoderLayer as TransformerEncoderLayer
...
class CorrBlock1D:
    def __init__(self, pixel_wise_net, pixel_wise_net_sp, ref_feats, multi_src_feats, depth_range, num_depth,
                 proj_l3, proj_l2,
                 num_levels=3, radius=4, G=8, ref_depth=None, training=True, identity=False, visibility=True,
        ...
        # 1 Initialize 1/8 dense correlation volume
        ...
        corr_sp, pair_view_weights = self.corr(pixel_wise_net_sp, ref_feat_l1, multi_src_feat_l1, proj_l2, depth_samps_topk, 
                                               pair_view_weights=None) # TODO debug
    ...
    def compute_interpolation_weights(self, rel_index):
        rel_index_f = torch.floor(rel_index)
        rel_index_c = rel_index_f + 1

        w0 = rel_index_c - rel_index
        w1 = rel_index - rel_index_f

        weights = torch.cat([w0, w1], dim=1)
        indices = torch.cat([rel_index_f, rel_index_c], dim=1)
...
class SDropout(nn.Module):
    def __init__(self, in_channels, thresh=3.91):
        '''Args:
    ...
    def gen_log_alpha(self, batch_size, input_):   ### NOTE Gaussian
    ...
    def activated_neurons(self):
        return self.clip_mask.sum() / self.clip_mask.numel()
...
class ConvGRU(SIGEModel): # nn.Module):
    ...
    def forward(self, h, motion_features, inp, offset_mask=None, prev_clip_mask=None, fast_infer=False, coords_grid=None):
        hx = torch.cat([h, motion_features, inp], dim=1) # TODO sparsify
        q_dense = None
...
class FeatScaleNet(nn.Module):
    ...
    def forward(self, feat, depth):
        ...
        if False:
            ...
            grid_h = torch.tensor(grid_h,dtype=torch.float,requires_grad=False, device=device).repeat(b,1,1,1)
            grid_w = torch.tensor(grid_w,dtype=torch.float,requires_grad=False, device=device).repeat(b,1,1,1)
...
class ChannelAttnNet(nn.Module):
    def __init__(self, input_dim=96, hidden_dim=32, output_dim=1):
        super(ChannelAttnNet, self).__init__()
        self.conv1 = nn.Conv2d(input_dim, hidden_dim, 3, padding=1, bias=False)
        self.conv2 = nn.Conv2d(hidden_dim, output_dim, 3, padding=1)
        self.relu = nn.ReLU(inplace=True)

    def forward(self, x):
        return self.conv2(self.relu(self.conv1(x))).sigmoid()
...
class BasicMultiUpdateBlock(nn.Module):
    ...
    def forward(self, net, inp, corr=None, flow=None, need_mask=True,
                offset_mask=None, prev_clip_mask=None, fast_infer=False, coords_grid=None):

        motion_features = self.encoder(flow, corr)
        
        net[0] = self.gru04(net[0], motion_features, inp, offset_mask=offset_mask, prev_clip_mask=prev_clip_mask,
                            fast_infer=fast_infer, coords_grid=coords_grid)
...
class ASMVSNet(nn.Module):
    def __init__(self, nscale=3, nsrc=4, lamb=1.5, n_depths=192, grad_method="detach", base_chs=[48, 32, 16], feat_ext_ch=8,
                ndepths=None, mindepth=None, mixed_precision=True, n_iters=16, n_iters_val=32, motion_dim=64,
                hidden_dims=[128, 64], n_gru_layers=3, corr_levels=3, corr_radius=4, n_downsample=1,
                need_conf=True, feat_mdconv=True, gru_mdconv=False, corr_visibility=True, use_attn_loss=False, attn_plus_one=False,
                lambda_ce=10, loss_annealing=False, lambda_kl=1e-4, sparse_dropout=False, sparse_infer=False):
        ...
        self.corr_visibility = corr_visibility
        self.attn_plus_one = attn_plus_one
        ...
        self.feature_extraction = FeatureNet(mdconv=feat_mdconv)
        self.context_extraction_v2 = ContextNet(mdconv=feat_mdconv) # out_channel=16
        d_model=48
    ...
    def compute_depth(self, ref_feat, src_feats, ref_context, depth_range, lamb, inverse_depth_base,
                      inverse_depth_multiplier, proj_l3, proj_l2, is_training=False, ref_depths=None,
                      coords_grid=None, homo_samp_coords=None, depth_samps_flat=None):
        n_iters = self.n_iters if self.training else self.n_iters_val

        with autocast(enabled=self.mixed_precision):

            # 1/8 dense corr, 192 hypos; 1/4 sparse corr, 16 hypos
            corr_fn = CorrBlock1D(self.pixel_wise_net, self.pixel_wise_net_sp, ref_feat, src_feats, depth_range, 
                                ...
                                visibility=self.corr_visibility, training=self.training, depth_samps_flat=depth_samps_flat,
                                res_div=True)
        ...
        with autocast(enabled=False):
            for itr in range(n_iters):
                ...
                plt.imsave('dep_itr{}.png'.format(itr), cur_depth_inv[0,0].cpu())
        ...
        if self.need_conf and self.training and (n_iters > 0):
            sigmoid_depth = (cur_depth_inv - inverse_depth_base) / inverse_depth_multiplier
            uncertainty_refined = self.uncertainty_refine_net_v2(net_feats[0], corr_fn.corr_full, conf, sigmoid_depth.float())
            uncertainty_refined = F.interpolate(uncertainty_refined, scale_factor=2**self.n_downsample, mode="bilinear", align_corners=False)
            pred_confs.append(uncertainty_refined)

        if self.training:
            return {"depth": pred_depths[-1], "confidence": uncertainty_refined, 'confidence_0': confidence_0,
                    'pred_depths': pred_depths, 'pred_confs': pred_confs, 'corr': corr_init, 'reg_loss_list': reg_loss_list}
        else:
            return {"depth": pred_depths[-1]}
    ...
    def forward(self, ref_img, imgs, proj_l3, proj_l2,\
                ref_proj_inv_l3, proj_invs_l3, depth_values, ref_depths=None,
                ref_depth_mask=None, ref_occ=None, epoch=None,
                homo_samp_coords=None, coords_grid=None, depth_samps_flat=None, depth_samps=None,
                n_iters=None):
        if n_iters is not None:
            self.n_iters_val = n_iters

        features = self.feature_extraction(imgs, proj_invs_l3, self.feature_transform_infer)
        ref_feature = {
        ...
        ref_context = self.context_extraction_v2(ref_img, ref_proj_inv_l3, self.context_transform)

        depth_range = depth_values

        with torch.no_grad():
            batch = ref_context.shape[0]
            inverse_depth_base = 1 / depth_range[:,1]   # B
        ...
        outputs = self.compute_depth(ref_feature, src_features, ref_context, depth_values,
                                     self.lamb, inverse_depth_base, inverse_depth_multiplier,
                                     proj_l3, proj_l2,
                                     is_training=self.training, ref_depths=ref_depths,
                                     coords_grid=coords_grid,
