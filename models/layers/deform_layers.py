# -*- coding: utf-8 -*-
# @Time    : 2020/12/12 22:09
# @Author  : Wang Xin
# @Email   : wangxin_buaa@163.com
# @File    : deform_layers.py

import torch
import torch.nn as nn
from .deform_conv import DeformConv, ModulatedDeformConv
from mvs.visualizers.utils.plot import *
...
class DeformConv2d(nn.Module):
    ...
    def forward(self, x):
        if self.modulation:
            ...
            # if True: # x.shape[1] == 36:
            #     vis_layer_pca(x, 'x.png')
            #     vis_layer_pca(offset, 'offset.png')
            #     vis_layer_pca(mask, 'mask.png')
            #     print('offset:', offset.abs().mean())
            #     print('x.shape =', x.shape, ', offset.shape =', offset.shape, ', offset.abs.median =', offset.abs().median())
            #     vis_deform(offset, mask, x.sum(dim=1, keepdim=True), (20,20), 'deform01.png')
            #     import time
            #     for i in range(30): vis_deform(offset[:,:18], mask, x.sum(dim=1, keepdim=True), (18+i,65), 'deform01.png'); time.sleep(0.3)
            #     vis_deform(offset[:,18:], mask, x.sum(dim=1, keepdim=True), (18,65), 'deform02.png')
...
class DeformConv2dAdp(nn.Module):
    """A single (modulated) deformable conv layer, with adaptive offset_mask"""
    ...
    def __init__(self, in_channels,
                 ...
                 double_mask=True,
                 bias=False):
        super(DeformConv2dAdp, self).__init__()
    ...
    def forward(self, x, offset_mask):
        if self.modulation:
            ...
            #     import time
            #     for i in range(30): vis_deform(offset[:,:18], mask, x.sum(dim=1, keepdim=True), (18+i,65), 'deform01.png'); time.sleep(0.3)
            #     vis_deform(offset[:,18:], mask, x.sum(dim=1, keepdim=True), (48,66), 'deform02.png')
            #     # import time; time.sleep(0.2)
            #     import pdb; pdb.set_trace()
